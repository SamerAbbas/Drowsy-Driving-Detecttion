...
import cvzone.PlotModule
from cvzone.PlotModule import LivePlot
import pyttsx3

plotY = LivePlot(640, 360, [0, 10],invert= True)
plotX = LivePlot(640, 360, [0, 10],invert= True)
earList = []
marList = []
# a = []
...
landmark_style = draw_utils.DrawingSpec((0,255,0), thickness=2, circle_radius=2)
connection_style = draw_utils.DrawingSpec((0,0,255), thickness=2, circle_radius=2)
...
capture = cv.VideoCapture('Test\FemaleNoGlasses.mp4') # INSERT VIDEO HERE
...
while True:
    ...
    if result:
        ...
        if outputs.multi_face_landmarks:
            ...
            earList.append(ratio)
            if len(earList) > 5:
                earList.pop(0)
                earAvg = sum(earList)/len(earList)
                framePlot = plotY.update(earAvg)
                cv.imshow("EAR", framePlot)
            ...
            if frame_count > min_frame:
                #Closing the eyes
                speech.say('Drowsy Alert ... Wake up')
                #speech.runAndWait()
            #     a.append(0)
            # else: 
            #     a.append(1)
            ...
            marList.append(ratio_lips)
            if len(marList) > 5:
                marList.pop(0)
                marAvg = sum(marList)/len(marList)
                framePlot = plotX.update(marAvg)
                cv.imshow("MAR", framePlot)
                #print(marAvg)

            if ratio_lips < 1.8:
                #Open his mouth
                speech.say('Drowsy Warning ... Take a rest')
                #speech.runAndWait()
        ...
        # print(a)
        cv.imshow("Drowsy Detection", image)
